...
「AI Cross-Repo Task Log」に記録する。`docs/AI_TASK_HISTORY.md` は**例外的に**使う。
...
- **ai-ops にのみ関わる変更**（collect/sync スクリプト・ai-ops 自身の workflow・`.claude/` の開発ツール等、
  consumer に届かない内部変更）＝例外ケース → `docs/AI_TASK_HISTORY.md` に追記（新しいエントリを先頭に）。
- 判断基準: 「consumer の手元に影響が届くか」。届くなら Notion、届かないなら AI_TASK_HISTORY.md。
...
> 記録の保証ではない（`stop_hook_active` で1回だけ block。`docs/AI_TASK_HISTORY.md` を更新していれば即通す）。
