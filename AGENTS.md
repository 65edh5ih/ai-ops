...
- outbox 提案由来の変更（取り込み PR）は提案元セッションが居ない。**レビュー待ちで止まった**取り込み PR は、
  ...
  **既知の穴**: 自動マージされた取り込み PR は扱うセッションも居ないので、この確認を行う人が誰も居ない。
  配布物の欠陥が consumer 同期 PR にだけ出た場合、それを読む者が居ないまま残る。自動マージの条件
  （ベース一致・常時層の増加が上限内）は「前提が変わっていないこと」しか見ておらず、**配布物としての
  正しさは見ていない**ため、この穴は条件を厳しくしても塞がらない。塞ぐなら consumer 同期 PR の
  Codex 指摘を拾う経路（`codex-review-inbox` 等）を別に用意する必要がある。
