...
このリポジトリ **ops-sync** は、全リポジトリ（consumer）共通の **運用ルール**・**共通インフラ（ファイル）**・
...
- consumer 側に配布済みのもの（AGENTS.md のマーカー区間・manifest 管理下のファイル）を
  **consumer 側で手編集しない**。直すときは必ず ops-sync 側で直す（consumer で直しても次回同期で上書きされる）。

## 何を ops-sync に入れるか（置き場所の振り分け）

> 「**これは共通か固有か**」という判断ルール自体は、各 consumer のエージェントが*作業中に*下すものなので、
> 共通ブロック（`AGENTS_COMMON.md` の「共通か固有かの判断」節）に置いて全 consumer へ配布している。
> ここ（ops-sync 内）には、**共通と判断したものを ops-sync の*どこに*置くか**だけを書く。
...
> ops-sync 内では `docs/<name>.md` を `../shared/docs/<name>.md` への symlink にしておけば、ops-sync 自身の
...
- **指摘があれば ops-sync 正本で直す**（`shared/` を再修正 → 新 PR → 再配布）。**consumer 同期 PR は手編集しない**
  （次回同期で上書きされる）。同一ファイルを配る性質上、1回の正本修正で全 consumer 分が直る。
- outbox 提案由来の変更（取り込み PR）は提案元セッションが居ない。**レビュー待ちで止まった**取り込み PR は、
  それを扱うセッション（オーナーのレビュー/マージ時）が同じ手順で行う。
  ...
  **マージ済み PR に未 resolve で残っている指摘**（＝配布済みの欠陥に誰も着手していない状態）を
  一覧とジョブサマリの警告に出す。**未対応が残っていても run は緑**（毎時の失敗メールで通知が
  読まれなくなるため。赤は「一覧が信用できない＝取得失敗」だけに割り当てている）。
  気づき口はメールではなく**一覧ファイル**なので、`.ops-sync/review-inbox.md`（ops-sync 自身の分は
  private の全体一覧）を読んで消化する（下記）。

## private consumer の内部をこのリポジトリに書かない

**ops-sync は public**。consumer 名（`consumers.txt` で公開済み）を超える情報——private consumer の
**PR 番号・欠陥の内容・修正のタイムライン・実装識別子**——を書かない（MUST NOT）。書いてよいのは
「ある private consumer で」「別の PR で」まで一般化した記述で、教訓（何が起きうるか・なぜその設計に
したか）はそれで十分残せる。

**対象は doc だけではない**（MUST）。`docs/history-inbox/` のフラグメント、**コミットメッセージ**、
**PR のタイトルと本文**——いずれも public な履歴として残るので同じ規約が効く。実際、配布 doc から
実例を履歴へ移して「隠した」つもりになり、次は PR 本文経由でコミットメッセージに同じ識別子を
書いた、という取りこぼしが起きている。**判断軸は「どの層に書くか」ではなく「このリポジトリが public か」**。
