...
対応済み**（詳細は `shared/docs/ops-sync-design.md`「前提・限界」のエージェント別入口一覧）。
...
| `.github/workflows/collect-outbox.yml` | （上り）cron（約3時間ごと）＋手動で提案を拾い、取り込みPR＋掃除PRを生成。あわせてトゥームストーン掃除 |
| `scripts/archive-task-history.mjs` | （保守）タスク履歴の保持量超過分を `docs/history-archive/` へ移す |
| `.github/workflows/archive-task-history.yml` | （保守）cron（1日1回）で ai-ops＋全 consumer を巡回し、超過分のアーカイブPRを生成・マージ |
...
- **手順系 doc（SOP）を書く・直す**: 書式は `shared/docs/sop-format.md`（consumer では `docs/sop-format.md`。
  ...
  ラッパーを添えると、Claude Code / Codex / OpenHands（V1）/ Gemini CLI / Antigravity が自動発火できる
  （各エージェント向けミラー `.codex` / `.openhands` / `.gemini` / `.agents`〔Antigravity〕は apply-shared が
  配布時に自動生成する。ai-ops に置くのは正本1ファイルだけ）。
...
- **タスク履歴のアーカイブ**: 自動。*Archive task histories* workflow（cron 1日1回）が ai-ops と
  全 consumer の `docs/AI_TASK_HISTORY.md` を巡回し、保持量（直近2作業日分 →
  `shared/docs/task-history.md`）を超えた分をアーカイブPRにして自動マージする。エージェントは
  追記だけでよい。急ぐときは workflow を手動実行する。
...
`docs/outbox-proposal.md`）。ai-ops の collect workflow（cron 約3時間ごと）が拾い、
