...
- 共通の配布物は3層: 常時ロード層（`AGENTS_COMMON.md`→各 AGENTS.md マーカー区間）、オンデマンド共通 doc
  （`shared/docs/*.md`→consumer `docs/*.md`）、バイト一致の実ファイル（`shared/**`）。下り配布も上り提案
  （`shared-file`）も `shared/docs/` を含めて稼働済み。散文 doc の横断的編集は Notion 経由で ai-ops に依頼する
  エスカレーション経路を用意（outbox 主・Notion 従。詳細は `shared/docs/OPS_SYNC_DESIGN.md`）。
