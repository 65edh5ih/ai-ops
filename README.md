...
| `scripts/collect-outbox.mjs` | （上り）consumer の `.ops-sync/outbox/*.md` 提案を種別に応じて反映（1 consumer 分をまとめて処理・全文置換の提案〔`common-block-edit`／`shared-file`〕はベースハッシュで鮮度検査・不正な提案は `rejected/` へ差し戻し） |
