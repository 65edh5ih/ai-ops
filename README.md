...
| `.github/workflows/sync.yml` | （下り）変更時＋cron（1日1回の再適用＝手編集ドリフトの自己修復）で各 consumer へ同期PRを自動生成（MERGE_MODE で自動マージ可）。配布前に `node --test` を通し、落ちれば配布しない |
...
| `.github/actions/create-pr-quiet/` | （内部）他リポジトリへ commit する PR の作成・更新。変更ファイル名を public な ops-sync のジョブログに出さないため、git 操作を `run:` に持ち込んで出力を詳細ログへ逃がす（consumer には配布しない） |
...
- **レビューの未対応在庫を見る**: 上の「マージ後の同期 PR を確認する」はセッションが生きている
  ...
  **未対応が残っていても run は緑**（マージ済み PR の未 resolve は warning とジョブサマリに出るだけ）。
  在庫は消化されるまで毎時再検出されるので、赤にすると同じ1件で毎時 run failed メールが飛び、通知が
  読まれなくなるため。**この workflow の赤は「取得に失敗した＝一覧が信用できない」だけを意味する**。
  気づき口はメールではなく一覧ファイルなので、上の2つを見る。
...
どちらでも**エージェントは自分でマージしない**。直接 PR はオーナーのマージで入る。outbox 経路では
...
（後続の提案は止まらない）。提案元が **private** リポジトリで、ベースハッシュが現在の正本と一致し、
常時層の増加も上限内なら、取り込み PR は**自動マージ**され、そのまま全 consumer へ配布される。
**提案元が public なら種別によらずレビュー待ち**（public には「main に載った＝オーナーが見た」が
成り立たないため。→ `shared/docs/ops-sync-design.md`「上りの認可」）。条件を満たさない提案が
1件でもあれば PR は open のまま残り、オーナーのレビュー待ちになる（run も失敗して知らせる）。
...
初回のインライン重複は手作業、上りは全文置換＝ベースハッシュで鮮度検査し一致したものだけ自動マージ、
collect は1回に 1 consumer 分＝衝突する提案は次回、など）。
