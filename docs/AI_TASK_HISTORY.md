...
## 2026-07-17 outbox 取り込みの cron を6時間→3時間に短縮（Actions 無料枠の実測に基づく）

nikki-san のセッションで「outbox の6時間間隔は何かの回数制限のためだったはず。もっと高頻度で
よいなら短くしたい」との依頼。制限の正体と余裕を実測してから間隔を決めた。

- **制約の正体は GitHub Actions の無料枠 2,000分/月**（このアカウントは全リポジトリ private の
  ため全 workflow が課金対象。job ごとに分単位切り上げ）。Cloudflare 側は日次制限で無関係。
- collect-outbox の実測は 20〜30秒/回＝課金1分/回。6時間おき≒120分/月 → 3時間おき≒240分/月で
  枠の1割強に収まる。**1時間おき（≒720分/月）は不採用**: nikki-san の開発日の実測が約170分/日
  あり、開発が集中する月はポーリングだけで枠超過の引き金になりうる。
- あわせて間隔の根拠を workflow コメントに明記し、「約6時間ごと」の記載（README・
  ops-sync-design・outbox-proposal）を追随させた。急ぎは従来どおり workflow_dispatch。
