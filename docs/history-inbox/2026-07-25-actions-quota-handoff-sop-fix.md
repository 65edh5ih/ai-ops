...
### 追記: 配布先(consumer)の同期PRにだけ出た指摘

ai-ops 本体の PR には出ず、**nikki-san#667 / private#421 の同期PRにだけ**追加指摘が出た
（`docs/ops-sync-design.md`「配布変更のダウンストリーム確認」が想定しているとおりの事象。実際に起きたので記録）。
...
学び: **配布 doc を変えたら consumer の同期PRのレビューまで見る**。同じファイルでも consumer 側は
別の AGENTS.md 文脈で読まれるため、ai-ops 側に出ない指摘が出る。今回は4件中3件がそれ由来（事実誤り1件を含む）。
