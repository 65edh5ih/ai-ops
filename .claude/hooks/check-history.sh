...
# 履歴記録（docs/history-inbox/ のフラグメント）に触れず完了しようとしたら、完了境界で1度だけ
...
cat <<'JSON'
{"decision":"block","reason":"作業履歴の記録がまだのようです（完了境界での確認）。scripts/new-task-history.mjs <タスクスラッグ> \"<短いタイトル>\" を実行し、作られた docs/history-inbox/ の新規ファイルへ今回の変更の「なぜ」（コードに無い制約・判断根拠）を記録してください（本体 AI_TASK_HISTORY.md には直接書かない）。記録、または『記録不要』の判断がついたら、そのまま完了して構いません（この確認は1回だけです）。"}
JSON
exit 0
