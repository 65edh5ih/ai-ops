...
# `docs/AI_TASK_HISTORY.md` は consumer に届かない ai-ops 内部変更だけの例外。
#
...
# 例外ケース（ai-ops 内部変更）として AI_TASK_HISTORY.md を更新済みなら即通す。
...
cat <<'JSON'
{"decision":"block","reason":"作業履歴の記録がまだのようです（完了境界での確認）。ai-ops は共通基盤そのものなので、今回の変更は通常 Notion『AI Cross-Repo Task Log』に「なぜ」を記録します。consumer に届かない ai-ops 内部だけの変更（collect/sync スクリプト・ai-ops 自身の workflow・.claude/ 等）なら例外的に docs/AI_TASK_HISTORY.md の先頭へ追記。記録、または『記録不要』の判断がついたら、そのまま完了して構いません（この確認は1回だけです）。"}
JSON
exit 0
