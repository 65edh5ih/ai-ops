...
- **エージェントに要る能力**（この仕組みはツール中立。以下の能力を*何で*満たすかはランタイム依存で、
  ...
  - 集約モードを使うなら追加で、**ai-ops をセッションから参照できる**こと。これは Claude Code on the web の
    ...
    別途持つ必要があり、無ければ**集約モードは使えない**（→ 下記モード節、既定の切り分け）。
...
- **実行リポジトリの選択（モード）**——どちらで起動するかで可視性と枠消費が決まる:
  ...
  - 現状の既定は **集約**（枠が逼迫しているため）。public に晒せない取得だけ分散にする。
    ...
    **その手段を持たないエージェントは、作業中の private リポジトリで完結する分散モードを既定にする**
    （集約を無理に使わない。可視性・機微性で選ぶ原則は下記のとおり保つ）。
...
2. **集約モードで ai-ops を使うなら、セッションから ai-ops を参照できるようにする**。エージェント起点で勝手に
   ...
   と同じ作法）。**この手段（別リポジトリのセッション追加）を持たないエージェントは集約モードを使えない**ので、
   分散モード（作業中リポジトリで完結）に切り替える——ただし取得内容が public に晒せない機微なら、public な
   集約経路をそもそも通さないための分散なので問題ない。機微でないのに集約を使いたいだけなら、ここで停止して
   ユーザーに集約実行（ai-ops への dispatch/読み取り）を依頼する（MUST）。
...
4. **workflow を起動する**。対象リポジトリの `net-fetch` workflow を `workflow_dispatch` で、
   `url` と一意の `request_id` を渡して実行する（`method` 既定 GET）。workflow を回す **git ref は対象
   リポジトリの既定ブランチ（通常 `main`）** を指す。**dispatch はエージェント自前の GitHub 資格情報で行う**
   ——具体的な手段はランタイム依存で、GitHub の MCP dispatch ツール・`gh workflow run`・REST の
   `POST .../actions/workflows/net-fetch.yml/dispatches` のどれでもよい（consumer 側に dispatch 用トークンを
   置かない＝ルール正本への増幅経路を作らない）。**REST を使うときは body に `ref`（既定ブランチ名。例 `main`）
   を含める**（MUST）——`ref` が無いと GitHub は 422 を返し run が作られない（`gh workflow run` は未指定なら既定
   ブランチを使うので省略可、MCP dispatch ツールは ref パラメータに既定ブランチを渡す）。**dispatch する手段が
   無ければ、ここで停止してユーザーに依頼する**（MUST。回避のために別経路を勝手に作らない）。
