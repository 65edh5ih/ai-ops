...
- **エージェントは自分でマージしない**（MUST NOT）。やるのは PR ないし提案を出すところまで。
  ...
  **どの経路でもオーナーのマージを最低1回通り、信頼境界をまたぐ経路では1回増える**
  ——通る場所と回数は経路で変わる:
  ...
  - **outbox 提案（private 発）** … オーナーが**提案ファイルを載せた consumer 側の PR** をマージする
    ...
    （条件は下記「取り込みの流れ」）＝合計1回。**同じ変更を二度承認させないための自動化**であって、
    承認を省いているのではない。機械確認に引っかかったものはオーナーのレビュー待ちになる。
  - **outbox 提案（public 発）** … consumer 側の PR に加えて、**ops-sync の取り込み PR も必ず
    オーナーがレビューしてマージする**＝合計2回。public には「main に載った＝オーナーが出所」が
    成り立たないため（→ `docs/ops-sync-design.md`「上りの認可」）。**2回目は冗長ではない**ので、
    自動マージされないことを異常と扱わない。
