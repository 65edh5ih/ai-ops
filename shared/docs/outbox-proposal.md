...
- collect は**1回の実行で最古の1件だけ**処理する。後続の提案は、先行の cleanup PR がマージされた後の
  実行で順に処理される（複数積んでも消えないが、届くのは直列）。
...
1. collect workflow が提案を1件処理し、**ai-ops への取り込み PR** と **提案元への outbox 掃除 PR** を生成。
