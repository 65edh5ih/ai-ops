...
そのスライスを齢に関係なく落とす。これによりブランチ上の滞在は TTL の3日ではなく実際の読み取り時間で
終わり、かつ**使ったときだけ 1 run** なので休眠中の課金がゼロになる（定期 sweep は使わない月も毎日
課金される）。日次 sweep は cleanup をし損ねたときのバックストップとして public にのみ残す。
...
cleanup が緑のまま何も消さない状態になる（public は TTL と日次 sweep が拾うが、private は次の取得まで
