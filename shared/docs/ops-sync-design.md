...
- **マージ済み PR の未 resolve を最上位に出す**（🔴）。配布済み＝全 consumer に欠陥が残っている状態で、
  最も危険なクラス。上り提案が自動マージされる経路では取り込み PR を扱うセッションが居ないため、
  この一覧が唯一の気づき口になる（→「上り（consumer 起点の提案）」）。open PR の指摘（🟡）は
  その PR を見ている者が居るので、そもそもこの強調の対象外。
  - **ただし 🔴 では run を失敗させない**（warning アノテーション＋ジョブサマリの警告ブロックまで）。
    在庫は消化されるまで**毎時**再検出されるので、失敗にすると同じ1件で毎時「run failed」メールが飛ぶ。
    気づかせるための信号が、通知を読ませなくする原因になっていた（実運用で「メールがうるさい」として
    停止要求が出た）。**未対応であること自体は異常ではなく在庫の残高**なので、赤ではなく残高として出す。
    件数はジョブサマリ・CI ログ・一覧ファイルの3箇所に残る。
  - **取得失敗（読めなかった対象）は今までどおり run を失敗させる**。こちらは在庫ではなく**収集機構の
    故障**で、放置すると短い一覧を「指摘ゼロ」と誤読する（fail-open）。毎時繰り返す性質のものでもない
    ため、メールが鳴っていい種類の赤。**赤の意味を「一覧が信用できない」に一本化する**のがこの分け方の
    ねらい（在庫の有無で色が変わらないので、赤＝要調査が保たれる）。
