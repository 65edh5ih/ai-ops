...
| 各リポジトリの `docs/AI_TASK_HISTORY.md` | 追記＝そのリポジトリのエージェント / アーカイブ移動＝ai-ops の archive CI（下記・保守バッチ） |
...
2. **特定タスクでのみ要る共通 doc** — 正本 `shared/docs/<name>.md`。consumer の `docs/<name>.md` へ配置。
   ...
   `.agents/skills/`〔Antigravity〕）は `apply-shared.mjs` が**配布時に正本から自動生成**する。
...
| `scripts/apply-shared.mjs` | （下り・ファイル）shared/tasks の配置＋manifest 差分による削除伝播＋skill ミラーの自動生成 |
...
| `.github/workflows/collect-outbox.yml` | （上り）cron（約3時間ごと）＋手動で起動、取り込み PR＋outbox 掃除 PR を生成。あわせてトゥームストーン掃除（保守） |
| `scripts/archive-task-history.mjs` | （保守）`docs/AI_TASK_HISTORY.md` の保持量超過分を `docs/history-archive/` へ移す |
| `.github/workflows/archive-task-history.yml` | （保守）cron（1日1回）で ai-ops＋全 consumer を巡回し、超過があればアーカイブ PR を生成・マージ |
...
consumer: エージェントが .ai-ops/outbox/<時刻>-<説明>.md を main に載せる
   └─ collect-outbox.yml（cron 約3時間ごと・手動可）が全 consumer を clone し、
...
各リポジトリの `docs/AI_TASK_HISTORY.md` の保持量超過分（→ `docs/task-history.md`）を
`docs/history-archive/<YYYY>.md` へ移す作業は、`archive-task-history.yml`（cron 1日1回）が
ai-ops＋全 consumer を巡回して行い、超過があるリポジトリにだけアーカイブ PR を生成・マージする。

- **エージェントのセッションでやらない**: 移動は完全に決定的で判断要素が無く、LLM（特に軽量モデル）に
  やらせるとトークンを浪費するうえ大きなファイル編集を壊しやすい（実際に力尽きる事例があった）。
...
- `shared/` 内の symlink は配布時に**実体化**される（`apply-shared.mjs` はファイル内容を読んでコピーする）。
  consumer には通常ファイルとして届く。skill の各エージェント向けミラーは `apply-shared.mjs` が正本
