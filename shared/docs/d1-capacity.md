...
「一度きりの移行が完了したか」「ある状態のD1行が本当に0件か」等は、**読み取り専用の SELECT を
本番に対してその場で実行して実証してから**、互換コード・防御的分岐の要否を判断すること（MUST）。
「このセッションから本番の実データを確認する手段がない」と結論する前に、そのランタイムで使える
読み取り専用の経路（MCP ツール・Cloudflare REST API・`wrangler d1 execute --remote` 等の CLI——
いずれも実装例であって手段を限定しない）が無いか確認する（MUST）。どの経路が使えるかはランタイムに
依るため、確認を1つの実装（例: Claude Code の Cloudflare MCP `d1_database_query`。`database_id` は
各リポジトリの `wrangler.toml` の `d1_databases` から取る）に決め打ちしない。書き込み系
（UPDATE/DELETE/INSERT）は本番データを直接変更する副作用があるため、このドキュメントの検証目的では
使わない（アプリの Worker/Function 経由で行う）。
