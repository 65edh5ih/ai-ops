...
//     （その repo 名）は、workflow が**警告**（warning アノテーション＋ジョブサマリ）を出す判定に使う。
//     一覧に積むだけでは**誰も見に行かない限り気づけない**ため（→ workflow の「Flag …」ステップ）。
//     **run は失敗させない**——在庫は消化まで毎時再検出され、赤にすると同じ1件で毎時メールが飛ぶ。
//     run を赤にするのは `failures`（＝収集機構の故障で一覧が信用できない状態）だけ。
//
// 終了コード: 常に 0（警告・失敗の判定は workflow 側で行う。ここで落とすと一覧の書き出しが飛ぶ）。
//
